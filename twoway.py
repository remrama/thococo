...
stat["mean(a)"] = a.mean()
stat["mean(b)"] = b.mean()
stat["median(a)"] = np.quantile(a, 0.5)
stat["median(b)"] = np.quantile(b, 0.5)
...
def significance_bars(ax, x1, x2, y, p, height=0.1, linewidth=1, caplength=None):
    ...
    ax.plot(
        x_coords, y_coords, color=color, linewidth=linewidth, transform=ax.get_xaxis_transform()
    )
    if caplength is not None:
        for x in [x1, x2]:
            ...
            cap_y = [y, y]
            ax.plot(
                cap_x, cap_y, color=color, linewidth=linewidth, transform=ax.get_xaxis_transform()
            )
    if stars:
        x_txt = (x1 + x2) / 2
        ax.text(
...
# Plot.
ax.errorbar(xvals, yvals, fmt="-s", yerr=yerrs, color="#091A60", linewidth=0.5, ms=6)
ax.set_xticks(xvals)
...
significance_bars(ax, x1=0, x2=1, y=0.9, p=p, height=0.02, caplength=None, linewidth=1)
...
ax.yaxis.set(
    major_locator=plt.MultipleLocator(major_yloc), minor_locator=plt.MultipleLocator(minor_yloc)
)
