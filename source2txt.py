...
def process_and_split(df, corpus_id, author_column, text_column, time_column=None):
    ...
    df.insert(0, "entry_id",
        df.groupby(author_column)[author_column].transform(lambda ser: range(ser.size)
            ).add(1).map(lambda x: f"entr-{x:d}")
    )
    df.insert(0, "author_id",
        df[author_column].map(lambda x: unique_authors.index(x)
            ).add(1).map(lambda x: f"auth-{x:d}")
    )
    df.insert(0, "corpus_id", f"corp-{corpus_id:s}")
    ### THIS APPLY LINE TAKES A LONG TIME AND LOTS OF MEMORY. DUMB.
    df["text_id"] = df.apply(lambda row: "_".join(row[["corpus_id", "author_id", "entry_id"]]), axis=1)
...
if __name__ == "__main__":
    ...
    #         ).reset_index()
    text_ser, attr_df = process_and_split(corpus, corpus_id, author_column, text_column, time_column)
